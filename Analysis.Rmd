...
Queer Responses Count

```{r}
table(d$role, d$Orientation == "LGBT")
...
queerp.vals = mutate(queerp.vals, sigval = ifelse(Value < 0.05, Value, "NS") )
queerSig$UGStudents <- queerp.vals$sigval
...
queerp.vals= tibble(Question = questionKey$Question[c(1:19)], Value = p.vals)
queerp.vals = mutate(queerp.vals, sigval = ifelse(Value < 0.05, Value, "NS") )
queerSig$GradStudents <- queerp.vals$sigval
...
```{r}
 options(warn=-1)
f <- d %>%
  filter(role == "Graduate" || role == "Undergraduate")
...
queerp.vals= tibble(Question = questionKey$Question[c(1:19)], Value = p.vals)
queerp.vals = mutate(queerp.vals, sigval = ifelse(Value < 0.05, Value, "NS") )
queerSig$AllStudents <- queerp.vals$sigval
```
